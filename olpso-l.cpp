...
#include<iostream>
#include"particles.h"
#include"E:\文件\程序\EC\OLPSO\OLPSO\fitness.h"
...
public:
	...
	PSO(int dim, int m, int Tmax, double max, double min, double c, double dt, double percent, int G, top Case)
	{
		this->Case = Case;
		this->dim = dim;
		this->m = m;
	...
	vector<double> getLbest(int i)
	{
		...
		case ring:
			int left = i - 1;
			int right = (i + 1) % particles.size();
			if (i == 0)
				left = particles.size() - 1;

			if (better(fitnessFunction(particles[left].pBest), fitnessFunction(particles[right].pBest)))
				lbest = particles[left].pBest;
			else
				lbest = particles[right].pBest;
			break;
	...
	void orthogonal(int n)
	{
		...
		vector<double>lbest = getLbest(n);
		static default_random_engine engine(time(nullptr));
...
void run(vector<double>& result1)
{
	...
	PSO pso = PSO(dim, m, Tmax, max, min, c, dt, percent, G, top);
	pso.getOA();
